    ...
    <Card className={className}>
      ...
      <CardContent className="flex flex-1 flex-col gap-4">
        ...
        <div className="flex flex-1 flex-col justify-evenly gap-4">
          {earningData.map((earningDatum, index) => (
            <div
              key={index}
              ...
              <div className="flex items-center justify-between gap-2.5">
                <Avatar className="size-11 rounded-sm">
                  <AvatarFallback className="bg-primary/10 shrink-0 rounded-sm">
                    <img
                      src={earningDatum.img}
                      alt={earningDatum.platform}
                      className="size-6"
                    />
                  </AvatarFallback>
                </Avatar>
                <div className="flex flex-col gap-1">
                  <span className="font-medium">{earningDatum.platform}</span>
                  <span className="text-muted-foreground text-sm">
                    {earningDatum.technologies}
                  </span>
                </div>
              </div>
              <div className="space-y-2">
                <p className="text-sm">{earningDatum.earnings}</p>
                <Progress
                  value={earningDatum.progressPercentage}
                  className="w-36"
                />
              </div>
            </div>
